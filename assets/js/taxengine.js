...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function performTaxComputation() {
        ...
        // House Property Interest Expense
        let hp_interest = sumAmounts(allTxs.filter(tx => tx.incomeHead === 'HP_Interest'));
        let net_hp = inc.hp - hp_interest;
        
        // Chapter VI-A Deductions Parsing
        let raw_80c = sumAmounts(allTxs.filter(tx => tx.incomeHead === 'Ded_80C'));
        let capped_80c = Math.min(raw_80c, 150000); // Strict 1.5L cap
        let d_80d = sumAmounts(allTxs.filter(tx => tx.incomeHead === 'Ded_80D'));
        let d_rest = sumAmounts(allTxs.filter(tx => ['Ded_80CCD', 'Ded_80E', 'Ded_80G', 'Ded_80TTA_TTB', 'Ded_Other'].includes(tx.incomeHead)));
        
        // Fallback for older entries saved just as 'Deductions'
        let legacy_ded = sumAmounts(allTxs.filter(tx => tx.incomeHead === 'Deductions'));
        d_rest += legacy_ded;

        let totalDeductions = capped_80c + d_80d + d_rest;
        ...
        let newGti = newNetSal + net_hp + inc.b_44ad + inc.b_speculation + inc.b_normal + inc.stcg + inc.ltcg + inc.os_interest + inc.os_dividend + inc.os_normal; 
        ...
        computedData = { inc, hp_interest, net_hp, salaryStdDed, capped_80c, raw_80c, d_80d, d_rest, totalDeductions, tds, oldGti, oldTotalInc, oldTax, oldRebate, oldCess, oldNetTax, oldPayable, newGti, newTotalInc, newTax, newRebate, newCess, newNetTax, newPayable };
    ...
    function renderClassicTable() {
        ...
        table.innerHTML = `
            ...
            <tbody>
                ...
                <tr><td class="ps-4">Gross Rental Income</td><td class="text-end">${f(computedData.inc.hp)}</td><td class="text-end">${f(computedData.inc.hp)}</td></tr>
                <tr><td class="ps-4">Less: Interest on Home Loan u/s 24(b)</td><td class="text-end text-danger">-${f(computedData.hp_interest)}</td><td class="text-end text-danger">-${f(computedData.hp_interest)}</td></tr>
                <tr class="fw-bold"><td class="ps-4">Net House Property Income</td><td class="text-end">${f(computedData.net_hp)}</td><td class="text-end">${f(computedData.net_hp)}</td></tr>
                
                <!-- BUSINESS -->
                <tr class="table-light"><td colspan="3" class="fw-bold">III. Profits & Gains of Business or Profession</td></tr>
                ...
                <tr><td class="ps-4">Intraday / Speculation Trading</td><td class="text-end">${f(computedData.inc.b_speculation)}</td><td class="text-end">${f(computedData.inc.b_speculation)}</td></tr>
                <tr><td class="ps-4">F&O Trading / Regular Business</td><td class="text-end">${f(computedData.inc.b_normal)}</td><td class="text-end">${f(computedData.inc.b_normal)}</td></tr>

                <!-- CAPITAL GAINS -->
                <tr class="table-light"><td colspan="3" class="fw-bold">IV. Capital Gains</td></tr>
                <tr><td class="ps-4">Short-Term Capital Gains (STCG)</td><td class="text-end">${f(computedData.inc.stcg)}</td><td class="text-end">${f(computedData.inc.stcg)}</td></tr>
                <tr><td class="ps-4">Long-Term Capital Gains (LTCG)</td><td class="text-end">${f(computedData.inc.ltcg)}</td><td class="text-end">${f(computedData.inc.ltcg)}</td></tr>

                <!-- OTHER SOURCES -->
                <tr class="table-light"><td colspan="3" class="fw-bold">V. Income from Other Sources</td></tr>
                <tr><td class="ps-4">Bank Deposit Interest (Savings / FD)</td><td class="text-end">${f(computedData.inc.os_interest)}</td><td class="text-end">${f(computedData.inc.os_interest)}</td></tr>
                <tr><td class="ps-4">Dividend & Miscellaneous Income</td><td class="text-end">${f(computedData.inc.os_dividend + computedData.inc.os_normal)}</td><td class="text-end">${f(computedData.inc.os_dividend + computedData.inc.os_normal)}</td></tr>
                ...
                <!-- DEDUCTIONS -->
                <tr class="table-light"><td colspan="3" class="fw-bold">VI. Deductions under Chapter VI-A</td></tr>
                <tr><td class="ps-4">Sec 80C (LIC, ELSS, PPF) - Capped 1.5L</td><td class="text-end text-danger">-${f(computedData.capped_80c)}</td><td class="text-end text-muted">NA</td></tr>
                <tr><td class="ps-4">Sec 80D (Health Insurance Premium)</td><td class="text-end text-danger">-${f(computedData.d_80d)}</td><td class="text-end text-muted">NA</td></tr>
                <tr><td class="ps-4">Sec 80CCD, 80E, 80G, 80TTA & Others</td><td class="text-end text-danger">-${f(computedData.d_rest)}</td><td class="text-end text-muted">NA</td></tr>
                <tr class="fw-bold"><td class="ps-4">Total Allowed Deductions</td><td class="text-end text-danger">-${f(computedData.totalDeductions)}</td><td class="text-end text-muted">NA</td></tr>
    ...
    async function generateComputationPDF() {
        try {
            ...
            const tableBody = [
                ...
                ['Gross Rental Income', f(c.inc.hp), f(c.inc.hp)],
                ['Less: Interest on Home Loan u/s 24(b)', `-${f(c.hp_interest)}`, `-${f(c.hp_interest)}`],
                [{ content: 'Net House Property Income', styles: { fontStyle: 'bold' } }, { content: f(c.net_hp), styles: { fontStyle: 'bold' } }, { content: f(c.net_hp), styles: { fontStyle: 'bold' } }],
                
                // BUSINESS
                [{ content: 'III. Profits & Gains of Business or Profession', colSpan: 3, styles: { fontStyle: 'bold', fillColor: [240, 240, 240] } }],
                ['Retail Business - Presumptive (Sec 44AD)', f(c.inc.b_44ad), f(c.inc.b_44ad)],
                ['Intraday / Speculation Trading', f(c.inc.b_speculation), f(c.inc.b_speculation)],
                ['F&O Trading / Regular Business', f(c.inc.b_normal), f(c.inc.b_normal)],

                // CAPITAL GAINS
                [{ content: 'IV. Capital Gains', colSpan: 3, styles: { fontStyle: 'bold', fillColor: [240, 240, 240] } }],
                ['Short-Term Capital Gains (STCG)', f(c.inc.stcg), f(c.inc.stcg)],
                ['Long-Term Capital Gains (LTCG)', f(c.inc.ltcg), f(c.inc.ltcg)],

                // OTHER SOURCES
                [{ content: 'V. Income from Other Sources', colSpan: 3, styles: { fontStyle: 'bold', fillColor: [240, 240, 240] } }],
                ['Bank Deposit Interest (Savings / FD)', f(c.inc.os_interest), f(c.inc.os_interest)],
                ['Dividend & Miscellaneous Income', f(c.inc.os_dividend + c.inc.os_normal), f(c.inc.os_dividend + c.inc.os_normal)],

                // GTI
                [{ content: 'Gross Total Income', styles: { fontStyle: 'bold' } }, { content: f(c.oldGti), styles: { fontStyle: 'bold' } }, { content: f(c.newGti), styles: { fontStyle: 'bold' } }],

                // DEDUCTIONS
                [{ content: 'VI. Deductions under Chapter VI-A', colSpan: 3, styles: { fontStyle: 'bold', fillColor: [240, 240, 240] } }],
                ['Sec 80C (LIC, ELSS, PPF) - Capped 1.5L', `-${f(c.capped_80c)}`, 'NA'],
                ['Sec 80D (Health Insurance Premium)', `-${f(c.d_80d)}`, 'NA'],
                ['Sec 80CCD, 80E, 80G, 80TTA & Others', `-${f(c.d_rest)}`, 'NA'],
                [{ content: 'Total Allowed Deductions', styles: { fontStyle: 'bold' } }, { content: `-${f(c.totalDeductions)}`, styles: { fontStyle: 'bold', textColor: [200, 0, 0] } }, 'NA'],
