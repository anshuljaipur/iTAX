    ...
    async function initializeTransactionUI() {
        ...
        txView.innerHTML = `
            <div class="tx-app-container">
                ...
                <div class="card p-4 mb-4 hidden border shadow-sm" id="txFormCard">
                    ...
                    <form id="txForm">
                        <div class="row g-3">
                            ...
                            <div class="col-md-4">
                                <label class="form-label">Income / Expense Head *</label>
                                <select class="form-select" id="txHead" required>
                                    ...
                                    <optgroup label="Income & Expenses: House Property">
                                        <option value="HouseProperty">Rental Income (Sec 22-27)</option>
                                        <option value="HP_Interest">Sec 24(b) - Home Loan Interest Expense</option>
                                    </optgroup>
                                    ...
                                    <optgroup label="Income from Capital Gains">
                                        <option value="STCG">Short-Term Capital Gain - Shares/Other</option>
                                        <option value="LTCG">Long-Term Capital Gain - Shares/Other</option>
                                    </optgroup>
                                    <optgroup label="Income from Other Sources">
                                        <option value="OS_Interest">Bank Deposit Interest - Savings/FD</option>
                                        <option value="OS_Dividend">Dividend Income from Shares</option>
                                        <option value="OS_Normal">Other Miscellaneous Income</option>
                                        <option value="Exempt">Exempt Income / Agricultural (Sec 10)</option>
                                    </optgroup>
                                    <optgroup label="Investments & Deductions (Chapter VI-A)">
                                        <option value="Ded_80C">Sec 80C (LIC, ELSS Mutual Fund, PPF, Tuition, Principal)</option>
                                        <option value="Ded_80D">Sec 80D (Health Insurance Premium & Checkup)</option>
                                        <option value="Ded_80CCD">Sec 80CCD (National Pension Scheme - NPS)</option>
                                        <option value="Ded_80E">Sec 80E (Interest on Education Loan)</option>
                                        <option value="Ded_80G">Sec 80G (Donations to Charitable Funds)</option>
                                        <option value="Ded_80TTA_TTB">Sec 80TTA/80TTB (Interest on Savings/FD)</option>
                                        <option value="Ded_Other">Other Deductions (Sec 80DD, 80U, etc.)</option>
                                    </optgroup>
                                    <optgroup label="Tax Payments">
                                        <option value="TDS">TDS / TCS / Advance Tax Paid</option>
                                    </optgroup>
                                </select>
                            </div>
                            <div class="col-md-2">
                                <label class="form-label">Category *</label>
                                <input type="text" class="form-control" id="txCategory" placeholder="e.g., SBI FD, LIC Premium" required>
                            </div>
                            <div class="col-md-2">
                            ...
                            <div class="col-md-12">
                                ...
                                <input type="text" class="form-control" id="txDescription" placeholder="Narration / Policy Number / Bank Name...">
                            </div>
    ...
    async function loadTransactionsForClient() {
        ...
        try {
            ...
            txs.forEach(tx => {
                ...
                if(tx.incomeHead.includes('Ded_')) badgeColor = 'bg-warning text-dark';
                if(tx.incomeHead === 'HP_Interest') badgeColor = 'bg-warning text-dark';
                if(tx.incomeHead === 'TDS') badgeColor = 'bg-danger';

                let displayHead = tx.incomeHead.replace('Ded_', '80-').replace('_', ' ');

                const tr = document.createElement('tr');
                tr.innerHTML = `
                    <td>${new Date(tx.date).toLocaleDateString('en-IN')}</td>
                    <td class="text-muted small">${tx.voucherNo}</td>
                    <td><span class="badge ${badgeColor}">${displayHead}</span></td>
                    <td>${tx.category}</td>
